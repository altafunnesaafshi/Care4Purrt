...
form.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    if (!data.ok) {
      msgBox.innerHTML = `<div class="alert alert-danger text-center">${data.error || 'Update failed'}</div>`;
      return;
    }

    msgBox.innerHTML = `<div class="alert alert-success text-center">${data.message || 'Health updated successfully'}</div>`;

  } catch (err) {
    msgBox.innerHTML = `<div class="alert alert-danger text-center">Network error. Please try again.</div>`;
  }
});
</script>
</body>
